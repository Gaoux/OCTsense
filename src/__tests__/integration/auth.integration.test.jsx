...
import { mockApi, mockCookies, restoreMocks } from '../utils/auth.integration.utils';
...
const AuthTestComponent = () => {
    ...
    return (
        <div>
            ...
            <span data-testid="email-valid">{validateEmail('test@example.com') ? 'válido' : 'inválido'}</span>
        </div>
    );
};
...
describe('Integración AuthContext', () => {
    ...
    it('logout limpia usuario y cookies', async () => {
        ...
        mockCookies({ user: JSON.stringify({ email: 'test@example.com', name: 'Test', id: '1', role: 'admin' }) });
        render(
            <AuthProvider>
                <AuthTestComponent />
            </AuthProvider>
    ...
    it('registro exitoso llama a la API y no lanza error', async () => {
        ...
        const TestRegister = () => {
            ...
            return (
                <button onClick={() => register({ email: 'nuevo@ejemplo.com', password: 'Password1', name: 'Nuevo', role: 'patient' })}>Registrar</button>
            );
        };
        render(
            <AuthProvider>
        ...
        await waitFor(() => {
            expect(registerMock).toHaveBeenCalledWith({ email: 'nuevo@ejemplo.com', password: 'Password1', name: 'Nuevo', role: 'patient' });
        });
    });
    ...
    it('actualización de usuario actualiza el estado', async () => {
        ...
        mockCookies({
            user: JSON.stringify({ email: 'test@example.com', name: 'Test', id: '1', role: 'admin' }),
            token: 'token123',
        });
        const TestUpdate = () => {
            ...
            return (
                <div>
                    <button onClick={() => login('test@example.com', 'password')}>Login</button>
                    <button onClick={() => updateUser({ name: 'Actualizado' })}>Actualizar</button>
        ...
        const TestRoles = () => {
            ...
            return (
                <div>
                    ...
                    <span data-testid="is-oftalmologo">{isOftalmologo() ? 'sí' : 'no'}</span>
    ...
    describe('Flujos de recuperación y verificación', () => {
        ...
        it('reset de contraseña exitoso redirige y muestra mensaje', async () => {
            ...
            axios.post.mockResolvedValue({ data: { message: 'Contraseña cambiada' } });
        ...
        it('verificación de email exitosa muestra mensaje', async () => {
            ...
            jest.spyOn(require('react-router-dom'), 'useLocation').mockReturnValue({ search: '?token=abc' });
            const navigate = jest.fn();
            jest.spyOn(require('react-router-dom'), 'useNavigate').mockReturnValue(navigate);
            render(<VerifyEmailPage />);
            await waitFor(() => {
                expect(screen.getByText('Tu correo fue verificado exitosamente.')).toBeInTheDocument();
            });
        });
    });
});
